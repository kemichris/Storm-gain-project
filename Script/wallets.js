...
const zcashContainer = document.getElementById("zcash");
const stellarContainer = document.getElementById("stellar");
const usdcContainer = document.getElementById("usdc");
const daiContainer = document.getElementById("dai");
const goldContainer = document.getElementById("gold");
const silverContainer = document.getElementById("silver");

const walletContainer = document.querySelector(".cold-wallet-flex");
const allWalletBtn = document.querySelector(".all-wallet-btn");

dashContainer.remove();
zcashContainer.remove();
stellarContainer.remove();
usdcContainer.remove();
daiContainer.remove();
goldContainer.remove();
silverContainer.remove();

let toggleWallet = true;

allWalletBtn.addEventListener("click", () => {
    toggleWallet? allWallet() : hideWallets();
    toggleWallet = !toggleWallet;
});


function allWallet() {
    walletContainer.insertBefore(dashContainer, walletContainer.children[7]);
    walletContainer.insertBefore(zcashContainer, walletContainer.children[8]);
    walletContainer.insertBefore(stellarContainer, walletContainer.children[9]);
    walletContainer.insertBefore(usdcContainer, walletContainer.children[10]);
    walletContainer.insertBefore(daiContainer, walletContainer.children[11]);
    walletContainer.insertBefore(goldContainer, walletContainer.children[12]);
    walletContainer.insertBefore(silverContainer, walletContainer.children[13]);

    allWalletBtn.innerHTML = "Hide wallets";
}
...
function hideWallets() {
    ...
    allWalletBtn.innerHTML = "All wallets";
}